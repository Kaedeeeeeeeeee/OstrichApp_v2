...
## ADR-007 · LLM provider 抽象（Anthropic + DeepSeek 双轨）

**日期**：2026-05-17
**状态**：✅ 已决定（紧急工程决策）

**背景**
Phase 1 e2e 验证时发现用户的 `ANTHROPIC_API_KEY` 字符串里混入了西里尔同形字（Cyrillic homograph，肉眼跟 ASCII 完全一样），导致 Anthropic API 直接 `401 invalid x-api-key`。在用户重新签发 key 之前 e2e 不能等。用户手上已有 DeepSeek 的 key，DeepSeek 又是 OpenAI 兼容协议且支持 function calling，最快的解封路径是让 `convex/claude.ts` 支持 provider 切换。

**决策**
`convex/claude.ts` 内部加 `LLM_PROVIDER` env 分发：

- `LLM_PROVIDER=anthropic`（默认，省略时也是这条路径）→ 现有 Anthropic SDK 行为完全不变，model=`claude-sonnet-4-7`
- `LLM_PROVIDER=deepseek` → OpenAI SDK，`baseURL=https://api.deepseek.com/v1`，model=`deepseek-chat`

`ostrichTools` 数组保持 Anthropic `{name, description, input_schema}` 形状作为 source of truth，DeepSeek 路径调用前在 `toolsToOpenAIFormat()` 里转换为 OpenAI 的 `{type: "function", function: {name, description, parameters}}`。响应解析时把 OpenAI 的 `tool_calls[].function.arguments`（JSON 字符串）`JSON.parse` 回内部统一的 `{ toolName, args }`，与 Anthropic 路径输出形状完全一致，`chat.ts::_appendOstrichReply` 等下游代码零改动。

**理由**

- 用户已有 DeepSeek key，能立刻解封 Phase 1 e2e
- `BLUEPRINT` 早就规划 LLM 可替换（虽然之前只支持 Anthropic）
- OpenAI SDK + `baseURL=api.deepseek.com/v1` 是 DeepSeek 官方推荐玩法
- 16 蛋 prompt + 工具 schema 与 provider 无关，转换是孤立的
- 默认仍走 Anthropic，主路径风险为零

**后果**

- ✅ Phase 1 demo 不被 Anthropic key 故障卡住
- ✅ 后续接其它 OpenAI 兼容模型（Kimi、Qwen、本地 vLLM）几乎零成本
- ❌ 维护两条调用路径（mitigated：工具 schema 转换孤立、两条路径都有 unit test 覆盖）
- ❌ DeepSeek 输出质量可能跟 Sonnet 4.7 不完全一致（Phase 1 可接受，只用于 demo 录像兜底）

**新增 env**

- `LLM_PROVIDER`：`anthropic`（默认）| `deepseek`
- `DEEPSEEK_API_KEY`：当 `LLM_PROVIDER=deepseek` 时必填，用户自配
