...
export function buildSystemPrompt(args: BuildSystemPromptArgs): string {
  ...
  const graphSummary =
    (args.graphSummary ?? "").trim().length > 0 ? args.graphSummary!.trim() : EMPTY_GRAPH;
  const memories = (args.memories ?? "").trim().length > 0 ? args.memories!.trim() : EMPTY_MEMORIES;
  const userLocalTime = args.userLocalTime ?? new Date().toISOString();

  return CHAT_SYSTEM_TEMPLATE.replace("{layer1_world}", WORLD_PROMPT)
